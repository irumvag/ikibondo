...
import { useAdminConsultations, useAdminCamps, QK } from '@/lib/api/queries';
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, [string, string]> = {
    OPEN:      ['var(--primary)', '#Open'],
    ESCALATED: ['var(--warn)', 'Escalated'],
    RESOLVED:  ['var(--success)', 'Resolved'],
  };
  const [color, label] = map[status] ?? ['var(--ink)', status];
  return (
    <span style={{
...
function ConsultationCard({ c, onRefresh }: { c: Consultation; onRefresh: () => void }) {
  ...
  const replyMut = useMutation({
    mutationFn: () => replyConsultation(c.id, replyText),
    onSuccess: () => { setReplyText(''); onRefresh(); },
  });

  const resolveMut = useMutation({
    mutationFn: () => resolveConsultation(c.id),
    onSuccess: onRefresh,
  });

  const disputeMut = useMutation({
    mutationFn: () => disputeConsultation(c.id),
    onSuccess: onRefresh,
  });
        ...
        <div style={{ padding: '0 16px 16px', borderTop: '1px solid var(--border)' }}>
          ...
          {c.messages && c.messages.length > 0 && (
            <div style={{ margin: '12px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {c.messages.map(m => (
                <div key={m.id} style={{
                  background: 'var(--bg)', borderRadius: 8, padding: '8px 12px', fontSize: 13,
                }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12, alignItems: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  ...
                  opacity: (!replyText.trim() || replyMut.isPending) ? 0.5 : 1,
                }}
              >
                <Send size={14} /> Reply
              </button>
...
const STATUS_LABELS: Record<string, string> = {
  '': 'All', OPEN: 'Open', ESCALATED: 'Escalated', RESOLVED: 'Resolved',
};
...
export default function AdminConsultationsPage() {
  ...
  const { data, isLoading, isError } = useAdminConsultations({
  ...
  function onRefresh() {
    qc.invalidateQueries({ queryKey: QK.adminConsultations() });
  }
  ...
  return (
    <div style={{ padding: '24px 20px', maxWidth: 860, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        <Inbox size={22} />
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>Consultations</h1>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <select
          ...
          style={{
            marginLeft: 'auto', border: '1px solid var(--border)', borderRadius: 8,
            padding: '5px 10px', background: 'var(--bg-elev)', color: 'var(--ink)', fontSize: 13,
          }}
        >
