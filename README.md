...
ml/
├── scripts/
│   ├── generate_synthetic_data.py  # Build the processed synthetic CSVs
│   ├── train_malnutrition.py       # Malnutrition risk classifier
│   ├── train_growth.py             # Growth trajectory forecaster
│   ├── train_vaccination.py        # Vaccination-dropout model
│   └── evaluate_all.py             # Cross-validation + metrics for all models
├── notebooks/          # Exploratory data analysis
├── data/               # data/raw/ is gitignored; processed synthetic CSVs are tracked
...
python scripts/generate_synthetic_data.py   # if data/processed is missing
python scripts/train_malnutrition.py
...
5. **Never commit**: `.env` files, ML model binaries (`.pkl`/`.joblib`), or media uploads
